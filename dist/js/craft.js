...
function displayAllCraft(){
  ...
  for (let z = 0; z< CraftList.length; z++) {
   cartonaa+=`<div class="craft_card bg-white p-6 rounded-2xl shadow-lg hover:scale-105 hover:bg-[#EE4F31] hover:text-white transition text-center w-7/12  md:w-5/12 lg:w-[26%] xl:w-[22%] relative">
            ...
            <div class="card-title-bar right-[-5px] left-[-5px] text-center py-2.5 px-5  bg-[#EE4F31] hover:bg-white hover:text-[#EE4F31] absolute top-[50%] transition">
              <h3 class="text-2xl font-semibold font-serif m-0">${CraftList[z].name}</h3>
            </div>
            <p class=" mt-10 transition">${CraftList[z].description.split(" ").slice(0,15).join(" ")}</p>
          <div class="mt-10">
